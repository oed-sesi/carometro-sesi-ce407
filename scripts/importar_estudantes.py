...
Funções principais:
  1. Sem argumentos      → cria template_estudantes.csv e template_estudantes.xlsx
  2. Com arquivo         → importa dados, gera alunos.json e organiza imagens por turma
  3. --organizar-imagens → apenas reorganiza fotos sem reimportar dados
...
Colunas do template (obrigatórias*):
  rm_estudante*, ra_estudante, nu_chamada, nome_estudante*, serie*, turma*, turno*,
  data_nascimento, email_estudante,
  rg_estudante, uf_rg, org_emissor_rg, cpf_estudante,
  status, data_ingresso,
...
Valores aceitos:
  ...
  uf_rg                            : Sigla do estado (ex: SP, RJ, MG)
"""
...
# ── Ordem exata das colunas conforme especificação ─────────────────────────────
COLUNAS_TEMPLATE = [
    "rm_estudante",
    "ra_estudante",
    "nu_chamada",
...
# Grupos visuais para o XLSX (separa por cor de cabeçalho)
GRUPOS_COLUNAS = {
    "Identificação Escolar": ["rm_estudante", "ra_estudante", "nu_chamada", "nome_estudante",
                              "serie", "turma", "turno"],
    "Dados Pessoais":        ["data_nascimento", "email_estudante",
                              "rg_estudante", "uf_rg", "org_emissor_rg", "cpf_estudante"],
    "Situação Escolar":      ["status", "data_ingresso"],
    "Responsáveis":          ["mae_nome", "mae_telefone", "mae_email",
                              "pai_nome", "pai_telefone", "pai_email"],
    "Endereço / Sistema":    ["endereco_estudante", "termo_autorizado",
                              "foto_coletada", "observacoes"],
}

DESCRICOES_COLUNAS = {
    "rm_estudante":      "Matrícula/RM do estudante (ex: 006810)  ★ OBRIGATÓRIO",
    "ra_estudante":      "Registro do Aluno SESI (ex: 000120628062-1)",
    "nu_chamada":        "Número de chamada na turma (ex: 01)",
    "nome_estudante":    "Nome completo do estudante (ex: ANA BEATRIZ SILVA SANTOS)  ★ OBRIGATÓRIO",
    "serie":             "Série: 6º Ano | 7º Ano | 8º Ano | 9º Ano | 1º EM | 2º EM | 3º EM  ★ OBRIGATÓRIO",
    "turma":             "Turma (ex: A | B | C)  ★ OBRIGATÓRIO",
    "turno":             "Turno: Manhã | Tarde | Integral  ★ OBRIGATÓRIO",
    ...
    "rg_estudante":      "Número do RG do estudante (ex: 12.345.678-9)",
    "uf_rg":             "UF emissora do RG (sigla, ex: SP)",
    "org_emissor_rg":    "Órgão emissor do RG (ex: SSP)",
    "cpf_estudante":     "CPF do estudante (ex: 123.456.789-00)",
...
LARGURAS_COLUNAS = {
    "rm_estudante": 14, "ra_estudante": 22, "nu_chamada": 12,
    "nome_estudante": 38,
    "serie": 12, "turma": 8, "turno": 12,
    "data_nascimento": 18, "email_estudante": 32,
    "rg_estudante": 18, "uf_rg": 8, "org_emissor_rg": 14, "cpf_estudante": 18,
    "status": 14, "data_ingresso": 18,
    "mae_nome": 30, "mae_telefone": 18, "mae_email": 28,
    "pai_nome": 30, "pai_telefone": 18, "pai_email": 28,
    "endereco_estudante": 40,
    "termo_autorizado": 18, "foto_coletada": 16, "observacoes": 42,
}
...
def formatar_cpf(val: str) -> str:
    """Garante formatação 000.000.000-00, aceita com ou sem formatação."""
    digits = re.sub(r"\D", "", val)
    if len(digits) == 11:
        return f"{digits[:3]}.{digits[3:6]}.{digits[6:9]}-{digits[9:]}"
    return val  # retorna original se inválido

def formatar_rg(val: str) -> str:
    """Mantém o RG como veio, apenas limpando espaços extras."""
    return val.strip()
...
def slugify_turma(serie: str, turma: str) -> str:
    ...
    s_lower = s.lower()
    tipo = "serie" if ("em" in s_lower or "série" in s_lower or "serie" in s_lower) else "ano"
...
def criar_template_xlsx(destino: Path) -> None:
    ...
    CORES_GRUPOS = {
        "Identificação Escolar": "C0392B",
        "Dados Pessoais":        "1A5276",
        "Situação Escolar":      "117A65",
        "Responsáveis":          "6E2F8E",
        "Endereço / Sistema":    "7D6608",
    }

    # Montar mapa coluna → grupo e cor
    col_grupo = {}
    for grupo, cols in GRUPOS_COLUNAS.items():
        for c in cols:
            col_grupo[c] = (grupo, CORES_GRUPOS[grupo])
    ...
    for col_idx, col in enumerate(COLUNAS_TEMPLATE, start=1):
        ...
        _, cor_hex = col_grupo.get(col, ("", COR_OPCIONA))
        cell = ws.cell(row=1, column=col_idx, value=col)
        cell.font      = Font(bold=True, color="FFFFFF", name="Calibri", size=10)
        cell.fill      = PatternFill("solid", start_color=cor_hex)
    ...
    for col_idx, col in enumerate(COLUNAS_TEMPLATE, start=1):
        ...
        cell.font      = Font(italic=True, color="5D6D7E", name="Calibri", size=9)
        cell.fill      = PatternFill("solid", start_color=COR_DESC)
    ...
    for col_idx, col in enumerate(COLUNAS_TEMPLATE, start=1):
        ...
        cell.font      = Font(color="1A5276", name="Calibri", size=10)
    ...
    # Larguras e alturas
    for col_idx, col in enumerate(COLUNAS_TEMPLATE, start=1):
        ws.column_dimensions[get_column_letter(col_idx)].width = LARGURAS_COLUNAS.get(col, 18)
    ...
    instrucoes = [
        ...
        ("Template de Importação de Estudantes  |  v5.3", True, "2C3E50"),
        ("", False, None),
        ("REGRAS GERAIS:", True, "C0392B"),
        ("• Preencha a partir da linha 4 da aba 'Estudantes' (linhas 1-3 são cabeçalho+exemplo)", False, "2C3E50"),
        ("• NÃO altere os nomes das colunas na linha 1", False, "C0392B"),
        ("• Campos obrigatórios (★): rm_estudante, nome_estudante, serie, turma, turno", False, "C0392B"),
        ("", False, None),
        ("NOVOS CAMPOS  (v5.3):", True, "1A5276"),
        ("• nu_chamada     → Número de chamada na turma (ex: 01)", False, "1A5276"),
        ("• rg_estudante   → RG do estudante (ex: 12.345.678-9)", False, "1A5276"),
        ("• uf_rg          → UF do RG (ex: SP)", False, "1A5276"),
        ("• org_emissor_rg → Órgão emissor do RG (ex: SSP)", False, "1A5276"),
        ("• cpf_estudante  → CPF do estudante (ex: 123.456.789-00)", False, "1A5276"),
        ("", False, None),
        ("FORMATOS:", True, "C0392B"),
        ("• Datas: DD/MM/AAAA  (ex: 15/03/2012)", False, "2C3E50"),
        ("• termo_autorizado / foto_coletada: Sim ou Não", False, "2C3E50"),
        ("• CPF: com ou sem formatação (o script formata automaticamente)", False, "2C3E50"),
        ...
        ("", False, None),
        ("FOTOS:", True, "C0392B"),
        ("• Fotos nomeadas como: <rm_estudante>.jpg  (ex: 006810.jpg)", False, "2C3E50"),
        ("• Coloque todas as fotos na pasta  images/  do projeto", False, "2C3E50"),
        ("• O script organiza automaticamente por turma após a importação", False, "2C3E50"),
        ("", False, None),
        ("CORES DAS COLUNAS:", True, "2C3E50"),
        ("• Vermelho  → Identificação Escolar  (RM, RA, Nº Chamada, Nome, Série, Turma, Turno)", False, "C0392B"),
        ("• Azul      → Dados Pessoais         (Nascimento, E-mail, RG, UF, Órgão, CPF)", False, "1A5276"),
        ("• Verde     → Situação Escolar        (Status, Data Ingresso)", False, "117A65"),
        ("• Roxo      → Responsáveis           (Mãe/Pai — Nome, Telefone, E-mail)", False, "6E2F8E"),
        ("• Amarelo   → Endereço / Sistema      (Endereço, LGPD, Foto, Observações)", False, "7D6608"),
    ]
    ...
    for row_idx, (texto, bold, cor) in enumerate(instrucoes, start=1):
        ...
        cell.font = Font(bold=bold, color=cor or "000000", name="Calibri",
                         size=12 if (bold and row_idx <= 2) else 11)
...
def ler_arquivo(caminho: Path) -> tuple[list[dict], list[str]]:
    ...
    if ext in (".xlsx", ".xlsm", ".xls"):
        ...
        # Pular linha de descrições (linha 2 do template)
        if len(df) >= 2:
            segunda = df.iloc[0]
...
def construir_aluno(row: dict, num_linha: int) -> dict | None:
    ...
    # CPF: formatar se tiver 11 dígitos
    cpf_raw = clean(row.get("cpf_estudante", ""))
    cpf = formatar_cpf(cpf_raw) if cpf_raw else ""

    return {
        # ── Identificação escolar ──────────────────────────────
        "matricula":        rm,           # legado — mantido para compatibilidade
        "rm_estudante":     rm,
        "ra_estudante":     clean(row.get("ra_estudante", "")),
        "nu_chamada":       clean(row.get("nu_chamada", "")),
        "nome_completo":    nome,
        # ── Dados da turma ─────────────────────────────────────
        "serie":            serie,
        "turma":            turma.upper(),
        ...
        # ── Documentos ─────────────────────────────────────────
        "rg_estudante":     formatar_rg(clean(row.get("rg_estudante", ""))),
        "uf_rg":            clean(row.get("uf_rg", "")).upper(),
        "org_emissor_rg":   clean(row.get("org_emissor_rg", "")).upper(),
        "cpf_estudante":    cpf,
        # ── Responsáveis ───────────────────────────────────────
...
def organizar_imagens(alunos: list[dict], verbose: bool = True) -> dict:
    ...
    for img_file in sorted(IMAGES_DIR.iterdir()):
        if not img_file.is_file() or img_file.suffix.lower() not in EXTENSOES_IMG:
            continue
        rm    = img_file.stem
        aluno = rm_map.get(rm)
        if not aluno:
        ...
        shutil.copy2(img_file, pasta / f"{rm}.jpg")
        copiadas += 1
    ...
    for aluno in alunos:
        ...
        encontrou  = any((IMAGES_DIR / f"{rm}{ext}").exists() for ext in EXTENSOES_IMG)
        if not encontrou:
            nao_encontradas.append(rm)

    return {"copiadas": copiadas, "nao_encontradas": nao_encontradas, "turmas": sorted(turmas_criadas)}

# ── Importar arquivo e gerar JSON ─────────────────────────────────────────────
def importar(caminho: Path) -> None:
    print(f"\n{SEP2}")
    print("  Carômetro Escolar — Importação de Estudantes  |  v5.3")
    print(SEP2)
    ...
    # Avisar sobre colunas novas ausentes (não bloqueante)
    novas = ["nu_chamada", "rg_estudante", "uf_rg", "org_emissor_rg", "cpf_estudante"]
    ausentes_novas = [c for c in novas if c not in headers]
    if ausentes_novas:
        print(f"  ℹ️  Colunas novas não encontradas no arquivo (serão deixadas em branco):")
        for c in ausentes_novas:
            print(f"       • {c}")
        print()

    # Filtrar linhas completamente vazias
    linhas = [r for r in linhas if clean(r.get("rm_estudante", ""))]
    print(f"  Linhas encontradas: {len(linhas)}")
    print()

    # Carregar JSON existente (compatível com legado)
    ...
    for i, row in enumerate(linhas, start=4):
    ...
    # Ordenar por série → turma → turno → nº chamada → nome
    ...
    def cmp_aluno(a, b):
        ...
        # Ordenar por número de chamada se disponível
        nc_a = a.get("nu_chamada", "") or ""
        nc_b = b.get("nu_chamada", "") or ""
        try:
            nc_a_int = int(nc_a); nc_b_int = int(nc_b)
            if nc_a_int != nc_b_int: return nc_a_int - nc_b_int
    ...
    stats = organizar_imagens(resultado)
...
def gerar_templates() -> None:
    print(f"\n{SEP2}")
    print("  Carômetro Escolar — Geração de Templates  |  v5.3")
    print(SEP2)
    gerou = False
    for path, fn in [(TMPL_CSV, criar_template_csv), (TMPL_XLSX, criar_template_xlsx)]:
        if not path.exists():
            fn(path); gerou = True
        else:
            print(f"  ℹ️  {path.name} já existe (não sobrescrito)")
    ...
    if gerou:
        ...
        print(f"\n  💡  Delete os arquivos e execute novamente para recriar.")
    print(f"{SEP}\n")
...
def apenas_organizar() -> None:
    ...
    if not JSON_PATH.is_file():
        print(f"  ❌  {JSON_PATH} não encontrado. Execute a importação primeiro.\n")
        sys.exit(1)
    with open(JSON_PATH, encoding="utf-8") as f:
        alunos = json.load(f)
    ...
    print(f"\n  {SEP}")
    print(f"  {'📁  Pastas':<30}: {len(stats['turmas'])}")
    print(f"  {'🖼️  Copiadas':<30}: {stats['copiadas']}")
    for t in stats["turmas"]:
        count = len(list((BASE / t).glob("*.jpg"))) if (BASE / t).exists() else 0
        print(f"       • {t}/ ({count} fotos)")
    print(f"  {SEP}\n")
...
def main() -> None:
    args = sys.argv[1:]
    if not args:
        gerar_templates(); return
    if args[0] in ("--organizar-imagens", "--organizar", "-o"):
        apenas_organizar(); return
    caminho = Path(args[0])
